...
http.createServer(function (req, res) {
  var matches = req.url.match(/^\/(\d)\/([a-z0-9A-Z]{0,32})\.([a-z]+)\??/);
  if (!matches) {
    invalidRequest(res, 'missing parameters');
    return;
  }
  var query = {};
  query.type = matches[1];
  query.md5 = matches[2];
  query.ext = matches[3];
  ...
  if (md5 === '2' || md5 === '1' || md5 === '0') {
    logger.debug('Using default local avatar cache to sendFile');
    sendFile(res, 'data/images/thumb/default/' + md5, contentType);
    return;
  }
  ...
  if (fs.existsSync('data/images/thumb/avatar/' + md5)) {
    logger.debug('Using local cache to sendFile');
    sendFile(res, 'data/images/thumb/avatar/' + md5, contentType);
    return;
  }
  ...
  instance.on('finish', function(){
    if (fs.existsSync(md5)) {
      ...
      fs.renameSync(md5, 'data/images/thumb/avatar/' + md5);
    } else {

...
NetManager.prototype.onNewPacket = function(packet) {
  switch(packet.packetHead.ctFlag) {
    ...
    case constant.ECtFlagConnectStates.CT_FLAG_KEEPALIVE_2: {
      // TODO(leeight)
      logger.warn('get keep alive packet from server, what should i do now?');
      break;
    }
...
NetManager.prototype.decode = function() {
  ...
  protocol.PacketFactory.getInstance().create(head,
    this._bytes.slice(packetHeadLength), function(err, opt_packet){
      if (err) {
        logger.error('invalid packet :-(');
        logger.error(err);
      } else {
        me._bytes = me._bytes.slice(length + packetHeadLength);
        logger.debug('After protocol.PacketFactory.create' + utils.dumpBuffer(me._bytes));

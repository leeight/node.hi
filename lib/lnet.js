...
NetManager.prototype.start = function() {
  ...
  socket.on('connect', this.startHandshake.bind(this));
...
NetManager.prototype._registerMessageHandlers = function() {
  ...
  this._messageHandlers['imagesvr'] = 'ImageSrvResponse';
...
NetManager.prototype.dispatchMessage = function(msg) {
  ...
  if (!res ||
      res.code === constant.StatusCode.SERVER_ERROR ||
      res.code === constant.StatusCode.IM_UNKNOWN) {
    logger.error('invalid res.code = [' + (res ? res.code : -999) + ']');
    return;
  }

  var protocolType = res.getProtocolType();
...
NetManager.prototype.startHandshake = function() {
  logger.debug("NetManager.prototype.startHandshake");

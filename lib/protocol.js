...
PacketHead.createFromBytes = function(bytes) {
  ...
  if (head.ctFlag === ECtFlagConnectStates.CT_FLAG_KEEPALIVE_2) {
    head.ctFlag = ECtFlagConnectStates.CT_FLAG_KEEPALIVE;
  }

  switch(head.nSendFlag) {
    case ECtSendFlags.CT_SEND_FLAG_HANDSHAKE:
    case ECtSendFlags.CT_SEND_FLAG_LOGIN:
    case ECtSendFlags.CT_SEND_FLAG_LOGOUT: {
      break;
    }
    default: {
      head.nSendFlag = ECtSendFlags.CT_SEND_FLAG_HANDSHAKE;
      break;
    }
  }
...
Packet.prototype.toString = function() {
  ...
  message.push(this.packetHead.toString());
  message.push(this.handshakeHead ? this.handshakeHead.toString() : '[handshakeHead = null]');
  message.push(this.handshakeBody ? this.handshakeBody.toString() : '[handshakeBody = null]');
  return message.join(':');
}
...
/**
 * 图片服务器的请求头.
 * @param {short=} opt_serviceType
 * @param {int=} opt_imid
 */
function RequestHeader(opt_serviceType, opt_imid) {
  /**
   * @type {short}
   */
  this.version = constant.RequestHeader.CURRENT_UDPRS_VERSION;
   ...
   * @type {short}
   */
  this.checkByte = constant.RequestHeader.CURRENT_CHECKBYTES;
   ...
   * @type {byte[12]}
   */
  this.reserve1 = new Buffer(12);
  this.reserve1.fill(0);

  this.serviceType = opt_serviceType || 0;
}

/**
 * @param {int} to_id
 */
RequestHeader.prototype.setToId = function(to_id) {
  this.serviceType = constant.RequestHeader.UPLOAD_OTHER_IMAGE;
  this.to_id = to_id;
}
 ...
 * @return {Buffer}
 */
RequestHeader.prototype.getBytes = function() {
  // 2: version
  // 2: serviceType
  // 2: checkByte
  // 2: reserved
  ...
  var bytes = new Buffer(2 * 4 + 4 * 4 + 12);
  bytes.fill(0);
  bytes.writeInt16LE(this.version, 0);
  bytes.writeInt16LE(this.serviceType, 2);
  bytes.writeInt16LE(this.checkByte, 4);
  bytes.writeInt16LE(this.reserved, 6);
  bytes.writeInt32LE(this.imid, 8);
  bytes.writeInt32LE(this.to_id, 12);
  bytes.writeInt32LE(this.busy, 16);
  bytes.writeInt32LE(this.redirSpeed, 20);
  this.reserve1.copy(bytes, 24, 0, this.reserve1.length);

  return bytes;
}

RequestHeader.prototype.toString = function() {
  var serviceTypeText = '<empty>';
  for(var key in constant.RequestHeader) {
    if (constant.RequestHeader[key] === this.serviceType) {
      serviceTypeText = key;
      break;
    }
  }
 ...
 */
RequestHeader.createFromBytes = function(bytes) {
  var packet = new RequestHeader();
  packet.version = bytes.readInt16LE(0);
  packet.serviceType = bytes.readInt16LE(2);
  packet.checkByte = bytes.readInt16LE(4);
  packet.reserved = bytes.readInt16LE(6);
  packet.imid = bytes.readInt32LE(8);
  packet.to_id = bytes.readInt32LE(12);
  packet.busy = bytes.readInt32LE(16);
  packet.redirSpeed = bytes.readInt32LE(20);
  packet.reserve1 = bytes.slice(24, 24 + 12);
...
/**
 * @constructor
 * @param {string=} opt_md5
 */
function ImageRequest(opt_md5) {
  /**
   * @type {string}
   */
  this.md5 = opt_md5 || '';
...
/**
 * @param {boolean=} opt_rmLastBit
 * @return {Buffer}
 */
ImageRequest.prototype.getBytes = function(opt_rmLastBit) {
  var rmLastBit = opt_rmLastBit || false;
  ...
  // 12: reserve
  // ??: this.a
  var byteLength = 16 + 4 + 4 + 12 + this.a.length;
  if (rmLastBit) {
    byteLength -= this.a.length;
  }

  var bytes = new Buffer(byteLength);
  utils.hex2bytes(this.md5).copy(bytes, 0, 0, 16);
  bytes.writeInt32LE(this.pos, 16);
  bytes.writeInt32LE(this.len, 20);
  this.reserve.copy(bytes, 24);
  if (!rmLastBit) {
    this.a.copy(bytes, 36);
  }
 ...
 * @return {ImageRequest}
 */
ImageRequest.createFromBytes = function(bytes) {
  var packet = new ImageRequest();
  packet.md5 = bytes.slice(0, 16).toString('hex');
  packet.pos = bytes.readInt32LE(16);
  packet.len = bytes.readInt32LE(20);
  packet.reserve = bytes.slice(24, 24 + 12);
  return packet;
}
...
ImageResponse.createFromBytes = function(bytes) {
  // 16: md5
  // 4: result
  // 4: pos
  // 4: len
  ...
  // 40: <total packet size>
  var packet = new ImageResponse();
  packet.md5 = bytes.slice(0, 16).toString('hex');
  packet.result = bytes.readInt32LE(16);
  packet.pos = bytes.readInt32LE(20);
  packet.len = bytes.readInt32LE(24);
  packet.reserve = bytes.slice(28, 28 + 12);
...
function ImagePacket() {
   ...
   * @type {int}
   */
  this.result = constant.IMAGE_RESULT_SUCCESS;
 ...
 */
ImagePacket.prototype.setHeader = function(header) {
  this.header = header;
}

/**
 * @param {ImageRequest} imageInfo
 */
ImagePacket.prototype.setImageInfo = function(imageInfo) {
  this.imageInfo = imageInfo;
}

/**
 * @param {ImageResponse} imageRes
 */
ImagePacket.prototype.setImageResponse = function(imageRes) {
 ...
 */
ImagePacket.prototype.setLocalFile = function(localFile) {
 ...
 * @param {Buffer} token
 */
ImagePacket.prototype.setToken = function(token) {
  this.token = token;
}
 ...
 * @param {int} result
 */
ImagePacket.prototype.setResult = function(result) {
  this.result = result;
}
 ...
 * @return {Buffer}
 */
ImagePacket.prototype.getBytes = function() {
  switch(this.header.serviceType) {
    case constant.RequestHeader.OTHER_IMAGE_AUTH:
    case constant.RequestHeader.DISPLAY_IMAGE_AUTH:
    case constant.RequestHeader.EMOTION_IMAGE_AUTH: {
      return utils.sumArray(this.header.getBytes(), this.token);
    }
    case constant.RequestHeader.UPLOAD_DISPLAY_IMAGE:
    case constant.RequestHeader.UPLOAD_EMOTION_IMAGE:
    case constant.RequestHeader.UPLOAD_OTHER_IMAGE: {
      try {
        this.imageInfo.a = require('fs').readFileSync(this.localFile);
        return utils.sumArray(this.header.getBytes(), this.imageInfo.getBytes());
      } catch(exp) {
        logger.warn(exp);
        return null;
      }
    }
    case constant.RequestHeader.QUERY_DISPLAY_IMAGE:
    case constant.RequestHeader.QUERY_EMOTION_IMAGE:
    case constant.RequestHeader.QUERY_OTHER_IMAGE: {
      return utils.sumArray(this.header.getBytes(), this.imageInfo.getBytes(true));
    }
    case constant.RequestHeader.DOWNLOAD_DISPLAY_IMAGE:
    case constant.RequestHeader.DOWNLOAD_EMOTION_IMAGE:
    case constant.RequestHeader.DOWNLOAD_OTHER_IMAGE: {
      return utils.sumArray(this.header.getBytes(), this.imageInfo.getBytes());
    }
    default: {
      return null;
...
function ImagePacketFactory() {
}
base.addSingletonGetter(ImagePacketFactory);

/**
 * @param {RequestHeader} head
 * @param {?ImageResponse} imageRes 有时候是null.
 * @param {Buffer=} opt_rawBuffer
 * @return {ImagePacket}
 */
ImagePacketFactory.prototype.create = function(head, imageRes, opt_rawBuffer) {
  var packet = null;
  switch(head.serviceType) {
    ...
    case constant.RequestHeader.OTHER_IMAGE_AUTH_RESP: {
      ...
      packet.setResult(opt_rawBuffer.readInt32LE(0));
      break;
    }

    case constant.RequestHeader.DOWNLOAD_DISPLAY_IMAGE_RESP:
    case constant.RequestHeader.DOWNLOAD_EMOTION_IMAGE_RESP:
    case constant.RequestHeader.DOWNLOAD_OTHER_IMAGE_RESP: {
      logger.debug('Download image successful. md5 = [%s], size = [%s]', imageRes.md5, imageRes.len);
      packet = new ImagePacket();
      packet.setHeader(head);
      packet.setImageResponse(imageRes);
      if (imageRes.len > 0) {
        // 读取数据，存储文件(一般就是大图的图片，头像之类的内容).
        // TODO(user) 存储本地文件，文件名如何获取呢？
        var fs = require('fs');
        fs.writeFileSync(imageRes.md5, opt_rawBuffer);
      } else {
        logger.error('Image file size = [%s]', imageRes.len);
      }
      break;
    }
...
exports.ImagePacket = ImagePacket;
exports.ImagePacketFactory = ImagePacketFactory;

...
ASN1.prototype.toHexDOM = function(opt_buffer) {
    var buffer = opt_buffer || [];
    buffer.push(this.stream.hexByte(this.stream.get(this.posStart())));
    for(var i = this.posStart() + 1, j = this.posContent(); i < j; i ++) {
        buffer.push((this.stream.get(i)));
    }
    if (!this.sub) {
        for(var i = this.posContent(), j = this.posEnd(); i < j; i ++) {
            buffer.push((this.stream.get(i)));
        }
    } else if (this.sub.length > 0) {
        var first = this.sub[0];
        var last = this.sub[this.sub.length - 1];
        for(var i = this.posContent(), j = first.posStart(); i < j; i ++) {
            buffer.push((this.stream.get(i)));
        }
        for(var i = 0, max = this.sub.length; i < max; ++i) {
            this.sub[i].toHexDOM(buffer);
        }
        for(var i = last.posEnd(), j = this.posEnd(); i < j; i ++) {
            buffer.push((this.stream.get(i)));
        }
    }

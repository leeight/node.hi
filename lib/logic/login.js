...
function Login() {
  ...
  this.on('_on_query_offline_msg_count', this._onQueryOfflineMsgCount.bind(this));
  this.on('message', this.onMessage.bind(this));
}
base.inherits(Login, events.EventEmitter);

/**
 * @param {response.OfflineMsgCountResponse} response
 * @param {lnet.NetManager} nm
 */
Login.prototype._onQueryOfflineMsgCount = function(response, nm) {
  nm.removeCommand(response.seq);
  switch (response.code) {
    case constant.StatusCode.SUCCESS: {
      // TODO(leeight) 如何处理呢?
      if (response.count > 0) {
        logger.info('offline msg count = [%s]', response.count);
        nm.getClient().emit('offline_msg_count', response.count);
      }
      break;
    }
    default: {
      logger.error('fetch offline msg count failed, code = [%s]', response.code);
      break;
    }
  }
}
...
Login.prototype._onLogin = function(response, nm) {
  ...
  switch(response.code) {
    case constant.StatusCode.SUCCESS: {
      ...
      // FIXME(leeight) We should not set imid here...
...
Login.prototype.onMessage = function(response, nm) {
  ...
  logger.debug('Login.prototype.onMessage, cmd = [' + cmd +

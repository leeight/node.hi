...
function Friend() {
  ...
  this.on('_on_contact_notify', this._onContactNotify.bind(this));
 ...
 */
Friend.prototype._onContactNotify = function(response, nm) {
  logger.debug('Friend.prototype._onContactNotify');
  var seq = response.seq;
  if (response.friend) {
    nm.getClient().emit('contact_notify', response.friend);
  }

 ...
 * @return {string}
 */
BaseResponse.prototype.getProtocolType = function() {
  if (this.command) {
    return (this.superCommand + '_' + this.command).toLowerCase();
  } else {
    return this.superCommand.toLowerCase();
  }
}
...
BaseResponse.createResponse = function(bytes) {
  ...
  logger.debug('xml = [' + JSON.stringify(response.xml) + ']');
  if (response.bodyData) {
    logger.debug('bodyData = [%s]', response.bodyData.toString('base64'));
  }
  ...
  response.superCommand = method[0].trim();
  response.version = method[1].trim();
  response.type = method[2].trim();
  response.seq = parseInt(method[3].trim(), 10);
  ...
  for(var i = 1; i < lines.length; i ++) {
    if (lines[i]) {
      ...
      if (chunks.length === 2) {
        response.responseHead[chunks[0]] = chunks[1].trim();
      } else {

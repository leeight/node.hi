...
ContactNotifyResponse.prototype.create = function() {
  ...
  this.friend.info_open_level = contact.getAttribute('info_open_level');
  this.friend.vitality = contact.getAttribute('vitality');
  ...
  if (avatar) {
    ...
    if (chunks.length === 3) {
      ...
      this.friend.avatar = this.friend.sex + '.jpg';
    }
  } else {
    this.friend.avatar = this.friend.sex + '.jpg';
  }

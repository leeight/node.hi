  ...
  switch(packet.header.serviceType) {
    ...
    case constant.RequestHeader.QUERY_OTHER_IMAGE_RESP: {
      if (packet.result === constant.IMAGE_RESULT_SUCCESS) {
        ...
        // 5. 发送UPLOAD_OTHER_IMAGE（如果文件比较大，应该分批次的上传）
        ...
        var chunks = Math.ceil(imageFileSize / constant.MAX_CT_BUFFER_SIZE);
        logger.debug('Upload image using #%s packets', chunks);

        // 分多个包上传
        for(var i = 0; i < chunks; i ++) {
          var info = new protocol.ImageRequest(image.md5);
          info.pos = i * constant.MAX_CT_BUFFER_SIZE;
          info.len = Math.min(constant.MAX_CT_BUFFER_SIZE, imageFileSize - constant.MAX_CT_BUFFER_SIZE * i);
          ...
          logger.debug('Sending packet #%s, pos = [%s], len = [%s]', i, info.pos, info.len);
          this.send(upImagePacket);
        }
      } else {

...
MsgRequestCommand.prototype._createImageMessage = function() {
  ...
  if (!fs.existsSync(image_path)) {
    logger.error('invalid image path = [%s], can\'t find it', image_path);
    return null;
  }
  ...
  if (display_message) {
    message.push('<text c="' + entities.encode(display_message + "\n", 0) + '" />');
  }
...
/**
 * @constructor
 * @param {int} imid 当前登录用户的Id.
 */
function ImageSrvCommand(imid) {
  BaseCommand.call(this, "IMAGESVR", "", "1.0");
  this.imid = imid;
  this.addCommandHead("from", this.imid)
}
base.inherits(ImageSrvCommand, BaseCommand);

ImageSrvCommand.prototype.createCommandBody = function() {
  return null;
}
...
exports.ImageSrvCommand = ImageSrvCommand;
